...
void to_dirchars(char *line, char *target);
int strcmp_dirorder(char *s1, char *s2, int folding);
int strcmp_dirorder_fold(char *s1, char *s2);
int strcmp_dirorder_nofold(char *s1, char *s2);
	...
	if (argc != 0)
		...
		if ((nlines = readlines(lineptr, MAXLINES)) >= 0) {
			my_qsort((void **) lineptr, 0, nlines-1, rev,
					(int (*)(void*,void*))(numeric ? numcmp : (dirorder ?  (folding ? strcmp_dirorder_fold: strcmp_dirorder_nofold) : (folding ? strcmp_lower : strcmp))));
...
int strcmp_dirorder_fold(char *s1, char *s2)
{
	return strcmp_dirorder(s1, s2, 1);
}

int strcmp_dirorder_nofold(char *s1, char *s2)
{
	return strcmp_dirorder(s1, s2, 0);
}
...
/* to_dirchars: copy line to target, but removing everything except letters, numbers, and blanks */
void to_dirchars(char *line, char *target)
{
	while (*line) {
